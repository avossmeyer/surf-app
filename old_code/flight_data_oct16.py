...
def scrape_skiplagged_bs(origin='LAX', dest='DPS', date=datetime.datetime.today().date() + datetime.timedelta(days=1)):
    HEADERS = ({'User-Agent':
                    'Mozilla/5.0 (X11; Linux x86_64) AppleWebKit/537.36 \
                    (KHTML, like Gecko) Chrome/44.0.2403.157 Safari/537.36', \
                'Accept-Language': 'en-US, en;q=0.5'})
    ...
    soup.find_all('div', class_='span2 trip-cost')
    import pdb;
    pdb.set_trace()


# <p class="" data-orig-price="852.1" data-orig-rate="1"><span class="">$852</span></p>
...
def scrape_expedia(origin='LAX', dest='DPS'):
    ...
    URL = 'https://www.expedia.com/Flights-Search?flight-type=on&mode=search&trip=oneway&leg1=from:({}),to:({}),departure:{}' + \
          'TANYT&options=cabinclass:economy&fromDate=9/3/2023&passengers=adults:1,infantinlap:N'.format(origin, dest,
                                                                                                        '9/3/2023')
...
def scrape_one_param(od):
    scrape_skiplagged_bs(od[0], od[1])
    ...
    for i, cur in app_data_df.iterrows():
        ...
        for home_airport in [['LAX', 'SAN', 'SFO', 'JFK', 'ORD', 'SYD', 'MEL', 'PER', 'AKL']]:
            ...
            if pd.isnull(cur['etl_insert_timestamp_outbound']):
                ...
                scrape_skiplagged(home_airport, destination_iata)
            ...
            if pd.isnull(cur['etl_insert_timestamp_return']):
                scrape_skiplagged_bs(destination_iata, home_airport)
